...
info = matrix(nrow = (Pblupgen + ssGBLUPgen+1), ncol  = 19)
info=data.frame(info)
colnames(info) = c("BV", "Coancestry","Heteroz", "SegAlleles", "SegQTL", "SegNeutral","F_drift_n","F_hom_n","F_drift_qtl","F_hom_qtl","F_drift_m","F_hom_m",
			"F_drift_n_gen","F_hom_n_gen","F_drift_qtl_gen","F_hom_qtl_gen","F_drift_m_gen","F_hom_m_gen","varA")
source("../Functions_MoBPS_simulations_23_11.R")

mapfile = paste("../../QMSim_Final_Solution/data_",args[1],".map",sep="")
pedfile = paste("../../QMSim_Final_Solution/data_",args[1],".ped",sep="")
freqfile = paste("../../QMSim_Final_Solution/plink_",args[1],".frq",sep="")
...
sam = sample(rownames(snps[snps$MAF>NeuMarkMAF,]),NeutralMarkers)
for (i in sam) {
...
effects = rgamma(qtl, shape = 0.4, scale = 1.7)
# plot(effects)
# hist(effects)
...
markerIncluded = rep(TRUE,length(IBDList))
for (i in qtlList) {
...
#                               }
   ...
   info[gen,9] = sum((p_qtl[gen,] - p_qtl[gen-1,])**2/(p_qtl[1,]*(1-p_qtl[1,])))/qtl
   info[gen,10] = 1-sum(2*(p_qtl[gen,]*(1-p_qtl[gen,]))/(2*(p_qtl[1,]*(1-p_qtl[1,]))))/qtl
   info[gen,11] = sum((p_marker[gen,] - p_marker[gen-1,])**2/(p_marker[1,]*(1-p_marker[1,])),na.rm=T)/sum(markerIncluded)
   info[gen,12] = 1-sum(2*(p_marker[gen,]*(1-p_marker[gen,]))/(2*(p_marker[1,]*(1-p_marker[1,]))),na.rm=T)/sum(markerIncluded)

   info[gen,13] = sum((p_neutral[gen,] - p_neutral[gen-1,])**2/(p_neutral[gen-1,]*(1-p_neutral[gen-1,])),na.rm=T)/length(p_neutral[gen-1,][p_neutral[gen-1,]!=1 & p_neutral[gen-1,]!=0])
   info[gen,14] = 1-sum(2*(p_neutral[gen,]*(1-p_neutral[gen,]))/(2*(p_neutral[gen-1,]*(1-p_neutral[gen-1,]))),na.rm=T)/length(p_neutral[gen-1,][p_neutral[gen-1,]!=1 & p_neutral[gen-1,]!=0])
   info[gen,15] = sum((p_qtl[gen,] - p_qtl[gen-1,])**2/(p_qtl[gen-1,]*(1-p_qtl[gen-1,])),na.rm=T)/length(p_qtl[gen-1,][p_qtl[gen-1,]!=1 & p_qtl[gen-1,]!=0])
   info[gen,16] = 1-sum(2*(p_qtl[gen,]*(1-p_qtl[gen,]))/(2*(p_qtl[gen-1,]*(1-p_qtl[gen-1,]))),na.rm=T)/length(p_qtl[gen-1,][p_qtl[gen-1,]!=1 & p_qtl[gen-1,]!=0])
   info[gen,17] = sum((p_marker[gen,] - p_marker[gen-1,])**2/(p_marker[gen-1,]*(1-p_marker[gen-1,])),na.rm=T)/length(p_marker[gen-1,][p_marker[gen-1,]!=1 & p_marker[gen-1,]!=0])
   info[gen,18] = 1-sum(2*(p_marker[gen,]*(1-p_marker[gen,]))/(2*(p_marker[gen-1,]*(1-p_marker[gen-1,]))),na.rm=T)/length(p_marker[gen-1,][p_marker[gen-1,]!=1 & p_marker[gen-1,]!=0])
...
if ((gen-Pblupgen-1) < GBLUPgens) {
     cohorts_to_use <- cohorts[-seq(2, to = Pblupgen*2+2, by = 2)]
